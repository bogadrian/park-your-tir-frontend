...
export const makeCallToServerLogin = async userData => {
  ...
  const urlLog = `https://bogdan-park-your-tir.herokuapp.com/api/v1/users/login`;
...
export const makeCallToServerSignUp = async userData => {
  //const env = runtimeEnv();
  const urlSign = `https://bogdan-park-your-tir.herokuapp.com/api/v1/users/signup`;
  const config = {
    headers: { 'Content-Type': 'application/json' }
  };
...
export const makeCallToServerUplod = async ({ photo, email, name }) => {
  ...
  const urlUpdate = `https://bogdan-park-your-tir.herokuapp.com/api/v1/users/updateMe`;
